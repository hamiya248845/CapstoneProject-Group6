...
const UserRegister = () => {
  const images = [bgimg01, bgimg02, bgimg03]; // Define the images array here
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (response.ok) {
        const responseData = await response.text(); // Get the response data
        console.log('Registration successful');
        console.log('Response Data:', responseData);
        navigate('/afterlogin'); // Use navigate to redirect
      } else {
